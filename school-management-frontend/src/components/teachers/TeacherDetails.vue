          ...
          {{ viewMode === 'add' ? 'View All Teachers' : 'Add Teacher' }}
            ...
            <div v-if="Object.keys(errors).length > 0" class="text-red-500 mt-2">
              <ul>
                ...
                <!-- Add additional error messages for other fields as needed -->
              </ul>
            </div>
            ...
            <li v-for="teacher in teachers" :key="teacher.id" class="mb-4">
              <div class="flex justify-between items-center bg-gray-200 p-4 rounded">
                ...
                <div class="flex items-center space-x-2">
                  <router-link :to="{ name: 'teacher-details', params: { id: teacher.id } }" class="text-indigo-500 hover:text-indigo-700">
                    Details
                  </router-link>
                  <button @click="editTeacher(teacher)" class="bg-yellow-500 hover:bg-yellow-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline-yellow active:bg-yellow-800">
                          Edit
                      </button>
                  <button
  ...
  data() {
    ...
    methods: {
        ...
        deleteTeacher ( teacherId )
        {
         ...
         editTeacher ( teacher )
    {
      // Switch to edit mode and populate the form with the teacher's data
      this.viewMode = 'edit';
    ...
    updateTeacher ()
    {
      ...
      // Make a PUT request to update the teacher
      api.put( `/api/teachers/${ this.newTeacher.id }`, this.newTeacher )
        .then( () =>
        {
          this.viewMode = 'view';
          this.newTeacher = {}; // Clear the form
          this.fetchTeachers(); // Refresh the teacher list
        } )
        .catch( error =>
        {
